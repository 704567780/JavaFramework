...
我之前在**小鹏汇天自动驾驶大数据部门**实习，我参与了“数据管理与监控系统”项目的开发，这个项目作为一个数据平台，对**数据进行解析处理**供自动驾驶其他部门使用，涵盖**数据展示平台(JAVA)**和**数据处理中心(Python)**和**数据备份及推送系统(Python)**，**数据展示平台**提供数据解析、展示等业务需求，**数据处理中心**作为处理模块负责多种解析需求。数据备份及推送系统负责数据检测、备份。
...
解决方向：Spring AOP做切面, ThreadLocal记录线程入口时间，before就开始设置当前时间，after就开始计算时间。计算完移除ThreadLocal。

知识点：[ThreadLocal](#如何理解ThreadLocal)、AOP
